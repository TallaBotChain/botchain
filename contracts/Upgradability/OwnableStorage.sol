...
contract OwnableStorage is KeyValueStorage, Ownable {

  mapping(address => bytes2) _contractIdMap;
  uint16 contractIdCount = 0;

  function setContractId(address contractAddress) public onlyOwner {
    bytes2 existingId = _contractIdMap[contractAddress];
    require (existingId.length == 0);
    contractIdCount = contractIdCount + 1;
    _contractIdMap[contractAddress] = bytes2(contractIdCount);
  }
  ...
  function senderIsValid() private view returns (bool) {
    return _contractIdMap[msg.sender].length != 0;
  }

  function scopedKey(bytes32 key) internal view returns(bytes32) {
    bytes32 contractId = _contractIdMap[msg.sender];
    require(contractId.length != 0);
    contractId = contractId << 30*8;
    bytes32 scoped_key = contractId | key;
    return scoped_key;
  }
