...
contract CurationCouncilRegistry is OwnableRegistry {
  ...
  */
  ...
  function incrementYayCount(uint256 registrationVoteId) private  {
  ...
  function incrementNayCount(uint256 registrationVoteId) private  {
  ...
  */
    ...
    public 
  {
    ...
    setInitialRegistrationVote(registrationVoteId, developerAddress);
  }

  function castRegistrationVote(uint256 registrationVoteId, bool vote) public {
    uint256 currentYayCount = 
    if vote {
      incrementYayCount(registrationVoteId)
    } else {
      incrementNayCount(registrationVoteId)
    }
  }

  /**
  * @dev Sets initial registration vote data
  * @param registrationVoteId An id associated with the registration vote
  * @param developerAddress address of developer requesting registration approval
  */
  function setInitialRegistrationVote(uint256 registrationVoteId, address developerAddress) private {
